...
void render_objects(sphere_t* spheres, size_t num_spheres)
{
    for (int y = camera.boundary.y0; y < camera.boundary.y1; ++y)
    {
        for (int x = camera.boundary.x0; x < camera.boundary.x1; ++x)
        {
            ...
            // "unproject" pixels by casting rays from the camera plane to its
            // origin
            ...
            for (size_t i = 0; i < num_spheres; ++i)
            {
                ...
                float    dist_hit;
                vec3u8_t color;
                hit_sphere(&ray, spheres[i], &does_intersect, &dist_hit,
                           spheres, num_spheres, &color);
                if (does_intersect && dist_hit < dist_closest)
                {
                    ...
                    pbuffer_write(x, y, color_closest.x, color_closest.y,
                                  color_closest.z);
                }
            }
        }
